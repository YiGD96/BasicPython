...
def Input() :
    # 성별
    global num
    gender_p = re.compile('^[M,F]$')
    gender_c = 0
    while not gender_c :
        gender = input('성별 >>> ')
        gender_c = gender_p.match(gender.upper())
        print(gender_c)
    # 이메일        @설정 해주기
    email_p = re.compile('^[a-z][a-zA-Z0-9]+@[a-z]+[.][a-z]{3,6}$')
    email_c = 0
    while not email_c:
        email = input('이메일 >>> ')
        ...
        print(email_c)
    # 출생년도
    birth_p = re.compile('^[^0][0-9]{3}$')
    birth_c = 0
    while not birth_c :
        birth = input('출생년도 4자리를 입력해주세요 >>> ')
        birth_c = birth_p.match(birth)
        print(birth_c)
    member = {'성별':gender,'이메일':email,'출생년도':birth}
    print(member)
    members[num] = member
    num += 1
    print(members)
